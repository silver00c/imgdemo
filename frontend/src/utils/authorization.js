...
async function authorization() {
    ...
    let hasLogin = false;
    let username = storage.getItem('username.myweb');

    const expiredTime = Number(storage.getItem('expiredTime.myweb'));
    const current = (new Date()).getTime();
    const refreshToken = storage.getItem('refresh.myweb');
    ...
    // 申请刷新 token
        ...
        try {
            ...
            storage.setItem('expiredTime.myweb', nextExpiredTime);
            storage.removeItem('refresh.myweb');
