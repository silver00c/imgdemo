...
<script>
    ...
    export default {
        ...
        mounted() {
            this.username = storage.getItem('username.myweb');
        },
        methods: {
            confirmDelete() {
                const that = this;
                authorization()
                    .then(function (response) {
                        if (response[0]) {
                            // 获取令牌
                            that.token = storage.getItem('access.myweb');
                            axios
            ...
            changeInfo() {
                ...
                authorization()
                    .then(function (response) {
                        ...
                        const oldName = storage.getItem('username.myweb');
                        // 获取已填写的表单数据
                        let data = {};
                        if (that.username !== '') {
                        ...
                        axios
                            ...
                            .then(function (response) {
                                ...
                                storage.setItem('username.myweb', name);
